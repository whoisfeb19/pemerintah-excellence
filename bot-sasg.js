...
// ✅ DAFTAR USER FIKTIF YANG HARUS DIHAPUS ---
    ...
    try {
        ...
        for (const userRecord of allUsersInDb) {
            ...
            try {
                ...
                // ✅ Kondisi 1: User ada di FICTIONAL_USERS list
                if (FICTIONAL_USERS.includes(discordId)) {
                    shouldDelete = true;
                    alasan = "USER FIKTIF (HARDCODED)";
                } else {
                    // Kondisi 2: Check user ada di Discord
                    let member = null;
                    try {
                    ...
                    // Kondisi 3: User tidak ada atau tidak punya role
                    if (!member) {
                        shouldDelete = true;
                        alasan = "TIDAK ADA DI DISCORD";
                    } else if (!member?.roles?.cache?.has(REQUIRED_ROLE_ID)) {
                        shouldDelete = true;
                        alasan = "TIDAK PUNYA REQUIRED ROLE";
                    }
                }

                // ✅ EXECUTE DELETE
                if (shouldDelete) {
                    console.log(`[CLEANUP-1] User ${discordId} (${namaUser}) - HAPUS [${alasan}]`);
...
// --- FUNGSI UNTUK PROSES FORUM LOGS (DENGAN VALIDASI STORAGE) ---
        ...
        for (let i = 0; i < logs.length; i++) {
            ...
            try {
                ...
                try {
                    ...
                    console.log(`[SUCCESS] Log ID ${log.id} + ${buktiGambarUrl ? '1 gambar' : 'tanpa gambar'} terkirim ke thread: ${namaUser}`);
                } catch (sendErr) {
                ...
                if (buktiGambarUrl) {
                    try {
                        const ambilNamaFile = buktiGambarUrl.split('/').pop();

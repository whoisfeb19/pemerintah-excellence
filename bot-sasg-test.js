...
<<<<<<< HEAD
        ...
        // Format isi post dengan deskripsi
        const content = `**San Andreas Police Department: Logs Kehadiran Bot - ${type.toUpperCase()}**\n\n${message}`;

        // Buat post (thread) di forum
...
=======
        ...
        console.log(`📤 Log dikirim ke forum: ${title}`);
>>>>>>> 651a01179c4d82fbb1ddc67a1ae3d2ffff5dc627
    } catch (err) {
    ...
    try {
        ...
        for (const user of allUsers) {
            ...
            if (!member || !member.roles.cache.has(REQUIRED_ROLE_ID)) {
                ...
                deletedUsers.push(`• **${user.nama_anggota}** (${user.discord_id}) - ${reason}`);
        ...
        if (deletedCount > 0) {
            ...
            // Kirim log bulk
            const logMessage = `**Penghapusan Users Tanpa Required Role**\n\n${deletedUsers.join('\n')}`;
            await sendLog(logMessage, 'delete');
        } else {
    ...
    try {
        ...
        for (const attendance of allAttendance) {
            ...
            if (!userExists || !memberInDiscord || !hasRequiredRole) {
                ...
                orphanedList.push(`• **${attendance.nama_anggota}** (${attendance.discord_id}) - ${reason}`);
        ...
        if (orphanedCount > 0) {
            ...
            // Kirim log bulk
            const logMessage = `**Pembersihan Data Orphaned Absensi**\n\n${orphanedList.join('\n')}`;
            await sendLog(logMessage, 'delete');
        } else {
        ...
        await sendLog(`Error cleanup orphaned: ${err.message}`, 'warning');
    }
}
...
async function runSasgTask() {
    ...
    try {
        ...
        await sendLog(`Sinkronisasi berhasil - ${dataToUpsert.length} users tersinkronisasi`, 'info');
        ...
        await sendLog(`Error utama: ${err.message}`, 'warning');
    }
}
